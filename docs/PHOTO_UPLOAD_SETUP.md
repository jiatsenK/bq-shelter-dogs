...
同一個服務也收「原始狗卡圖」：網站詳細資訊的狗卡資訊區按「上傳狗卡圖」，會存到 `cards/{編號}.jpg`，檢查方式和照片完全一樣（匿名、JPEG、2 MB、頻率限制和照片共用）。提交說明寫「上傳狗卡圖」或「更換狗卡圖」。

這個功能只要照「更新服務程式」重新貼一次 `worker/src/index.js` 就好，不用新增任何 Secret。
